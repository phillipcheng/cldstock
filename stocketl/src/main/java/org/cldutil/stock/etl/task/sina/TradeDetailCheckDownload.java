...
import org.cldutil.hadooputil.WholeFileInputFormat;
...
public class TradeDetailCheckDownload {
	...
	public static String launch(CrawlConf cconf, String datePart){
		String inDir = ETLConfig.RAW_ROOT + "/" + datePart + "/" + SinaETLConfig.SINA_STOCK_TRADE_DETAIL;
		String outDir = ETLConfig.CHECK_ROOT + "/" + SinaETLConfig.SINA_STOCK_TRADE_DETAIL + "/" + datePart;
		...
		try {
			...
			if (cconf.getYarnAppCp()!=null){
				for (String s: cconf.getYarnAppCp()){
			...
			if (cconf.getHadoopJobTracker()!=null){
				job.submit();
			}else{
