package org.cldutil.stock.etl.task;
...
import org.cldutil.hadooputil.HadoopUtil;
...
public class MergeTask extends Task implements Serializable{
			...
			if (partNames!=null){
				...
				if (partNames.length>1){
					...
					for (String partName:partNames){
						...
						Path destFile = null;
						if (!needOverwrite){
							destDir = new Path(ETLConfig.MERGE_ROOT + "/" + storeid + "/" + partName + "/" + datePart + "/" + midPart + "/tmp/");
							destFile = new Path(ETLConfig.MERGE_ROOT + "/" + storeid + "/" + partName + "/" + datePart + "/" + midPart + "/merge");
						}else{
							destDir = new Path(ETLConfig.MERGE_ROOT + "/" + storeid + "/" + partName + "/" + midPart + "/tmp/");
							destFile = new Path(ETLConfig.MERGE_ROOT + "/" + storeid + "/" + partName + "/" + midPart + "/merge");
						}
					...
					Path dest = null;
					if (!needOverwrite){
						dest = new Path(ETLConfig.MERGE_ROOT + "/" + storeid + "/" + datePart + "/" + midPart + "/merged");
					}else{
						dest = new Path(ETLConfig.MERGE_ROOT + "/" + storeid + "/" + midPart + "/merged");
					}
					if (fs.exists(dest)){
	...
	public static String[] launch(ETLConfig sc, String propfile, String marketId, CrawlConf cconf, String datePart, 
		...
		try {
			...
			Path fromDir = new Path(ETLConfig.RAW_ROOT+"/"+datePart);
			...
			if (doMR){
				...
				jobIdList.add(TaskUtil.hadoopExecuteCrawlTasks(propfile, cconf, tl, taskName, false, hadoopJobParams));
